...
const translations = {
  en: {
    sectionTitlePlaceholder: "e.g., SECTION - D (Long/Essay Questions)",
    addSection: "Add New Long Question Section",
    addQuestion: "Add Long Question",
    questionPlaceholder: "Enter long/essay question statement...",
    marks: "Marks",
    qPrefix: "Long Q# ",
    questions: "Questions",
  },
  ur: {
    sectionTitlePlaceholder: "مثال: حصہ چہارم - تفصیلی سوالات",
    addSection: "نیا تفصیلی سوالات کا سیکشن شامل کریں",
    addQuestion: "سوال شامل کریں",
    questionPlaceholder: "تفصیلی سوال کا متن درج کریں...",
    marks: "نمبر",
    qPrefix: "سوال نمبر ",
    questions: "سوالات",
  },
...
export function LongQuestionSection({ lang }: { lang: "en" | "ur" }) {
        ...
        <Card key={section.id} className="rounded-lg border shadow-none">
          ...
          <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
            <div className="flex-1 space-y-2">
              <Label
                htmlFor={`long-title-${sectionIndex}`}
                className="text-sm font-medium"
              >
                {isUrdu ? "سیکشن کا عنوان (اختیاری)" : "Section Title (Optional)"}
              </Label>
              <Input
                id={`long-title-${sectionIndex}`}
                {...register(`longQuestionSections.${sectionIndex}.title`)}
          ...
          <CardContent className="pt-6">
            ...
            <LongQuestionList sectionIndex={sectionIndex} lang={lang} />
          </CardContent>
        </Card>
      ))}

      {/* Button to add a new Long Question Section */}
      <Button
        type="button"
        variant="outline"
...
function LongQuestionList({
  ...
  const {
    ...
    control,
    name: `longQuestionSections.${sectionIndex}.questions`,
  });
  ...
  return (
    <div className="space-y-4">
      ...
      {questionFields.map((question, qIndex) => {
        ...
        const err = errors.longQuestionSections?.[sectionIndex]?.questions?.[qIndex];
          ...
          <div key={question.id} className="space-y-3 rounded-md border p-4">
            ...
            <div className="flex flex-col gap-4 sm:flex-row">
              ...
              <div className="flex-1 space-y-1.5">
                <Textarea
                  rows={3}
                  {...register(`longQuestionSections.${sectionIndex}.questions.${qIndex}.question`)}
              ...
              <div className="flex w-full flex-col gap-1.5 sm:w-28">
                <Label
                  htmlFor={`long-marks-${sectionIndex}-${qIndex}`}
                  className="text-xs font-medium text-muted-foreground"
                >
                  {t.marks}
                </Label>
                <Input
                  id={`long-marks-${sectionIndex}-${qIndex}`}
                  {...register(`longQuestionSections.${sectionIndex}.questions.${qIndex}.marks`)}
                  placeholder="5"
                  className="rounded-md"
                  dir={isUrdu ? "rtl" : "ltr"}
      ...
      <Button
        ...
        size="sm"
        className="rounded-md"
        onClick={() => appendQuestion({ question: "", marks: "5" })} // Defaults to 5 marks
      >
